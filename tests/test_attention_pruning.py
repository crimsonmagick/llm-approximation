...
class TestLlamaAttentionPruning(unittest.TestCase):
    
    def test_output_repeat_kv_pruned_uniform(self):
        ...
        # Validate outputs
        self.assertTrue(torch.equal(output_repeat_kv, output_repeat_kv_pruned), "Outputs are not equivalent")
        self.assertTrue(output_repeat_kv.shape == (batch, num_key_value_heads * n_rep, seqlen, head_dim), "Shape mismatch")
