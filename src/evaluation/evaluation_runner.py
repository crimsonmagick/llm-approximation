...
from src.models.model_resolution import get_model, LLMType
...
class EvaluationRun:
    ...
    def __init__(self, model_path: str, dataset_path: tuple,
                 evaluation_row_count: int = 10, scenario_name: str = 'default',
                 supports_attn_pruning: bool = False, device='cuda',
                 num_runs=1, batch_size=5, *, llm_type: LLMType, label, suite):
    ...
    def evaluate(self, *, input_ids, attention_mask):
        model = get_model(self.llm_type, self.model_path,
        ...
        for run_idx in range(self.num_runs):
            ...
            for batch_index in range(num_batches):
                logger.info(
                    f"{self.scenario_name}-{self.label}: Evaluating run={run_idx}/{self.num_runs}, batch={batch_index + 1}/{num_batches}")
    ...
    def _get_model(self):
        return get_model(self.llm_type, self.model_path,
                         self.supports_attn_pruning)
...
class PrunedEvaluation(EvaluationRun):

    def __init__(self, layer_idx: int, head_idxs: List[int], **kwargs):
...
class EnergyEvaluation(EvaluationRun):
    ...
    def evaluate(self, **kwargs):
        ...
        metrics_manager.accept_energy(captured_metrics, suite=self.suite)

        return predicted

class PerplexityEvaluation(EvaluationRun):
    ...
    def evaluate(self, **kwargs):
        ...
        metrics_manager.accept_perplexity(captured_metrics, suite=self.suite)
